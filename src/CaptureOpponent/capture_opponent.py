...
# Custom piece values for the Capture Opponent
...
# Maximum acceptable material loss in a trade sequence
MAX_TRADE_LOSS = 1
...
class CaptureOpponent(ChessEngineTemplate):
    """
    An engine obsessed with clearing the board of material.
    
    Uses Static Exchange Evaluation (SEE) to calculate multi-move trade sequences
    and aggressively pursues any trade that doesn't lose more than -1 material.
    """
    ...
    def _static_exchange_evaluation(self, board: chess.Board, move: chess.Move) -> int:
        """
        Static Exchange Evaluation (SEE) - calculate the material outcome of a capture sequence
        
        This simulates all possible recaptures on a square to determine the final material balance.
        """
        assert self.piece_values is not None
        ...
        # Get the piece values involved
        victim = board.piece_at(move.to_square)
        if victim is None:
            victim_value = self.piece_values[chess.PAWN]  # En passant
        else:
        ...
        while True:
            ...
            for attacker_move in board.legal_moves:
                if attacker_move.to_square == target_square:
                    piece = board.piece_at(attacker_move.from_square)
                    if piece:
                        piece_value = self.piece_values[piece.piece_type]
                        if piece_value < smallest_value:
                            smallest_value = piece_value
                            smallest_attacker = attacker_move
    ...
    def _is_good_trade(self, board: chess.Board, move: chess.Move) -> bool:
        """
        Determine if a capture is a "good trade" by our standards
        
        A good trade:
        - Removes material from the board
        - Doesn't lose more than -1 material
        - Uses SEE for complex sequences (2+ recaptures possible)
        """
        assert self.piece_values is not None
        ...
        # Count potential recaptures
        board.push(move)
        can_recapture = any(m.to_square == move.to_square for m in board.legal_moves)
        recapture_count = sum(1 for m in board.legal_moves if m.to_square == move.to_square)
        board.pop()
        
        # Simple capture (no recaptures) - always good!
        if not can_recapture:
            return True
        
        # Single recapture possible - check if it's an acceptable trade
        if recapture_count == 1:
            net_loss = attacker_value - victim_value
            return net_loss <= MAX_TRADE_LOSS
        
        # Multiple recaptures possible - use SEE
        see_value = self._static_exchange_evaluation(board, move)
        
        return see_value >= -MAX_TRADE_LOSS
    
    def _evaluate_position(self, board: chess.Board) -> int:
        """
        Evaluate position from the Capture Opponent's perspective
        ...
        # Check for checkmate
        if board.is_checkmate():
            return CHECKMATE_BONUS if board.turn == chess.WHITE else -CHECKMATE_BONUS
        
        # Check for stalemate or insufficient material (acceptable outcomes)
        ...
        material_removed = self.starting_material - total_material
        score += material_removed * MATERIAL_REMOVAL_BONUS
        
        # SECONDARY GOAL: Don't fall too far behind in material
        material_balance = self._calculate_material_balance(board)
        if material_balance < -MAX_TRADE_LOSS:
            penalty = (abs(material_balance) - MAX_TRADE_LOSS) * 25
            score -= penalty
        else:
...
def main():
    ...
    print("info string Capture Opponent v1.0 - Strategic Simplifier", file=sys.stderr)
    print("info string Goal: Remove all material from the board!", file=sys.stderr)
    print("info string Strategy: Accept trades losing up to -1 material", file=sys.stderr)
    print("info string Piece values: P=2 N=4 B=3 R=5 Q=8", file=sys.stderr)
    sys.stderr.flush()
